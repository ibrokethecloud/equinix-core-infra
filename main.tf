...
resource "metal_device" "harvester_dhcp" {
    ...
    project_id = data.metal_project.project.id
    user_data = templatefile("${path.module}/cloud-init.tpl", { dhcp_conf_b64 = base64encode(local.dhcp_conf), node_name = "harvester-dhcp", address = cidrhost(metal_reserved_ip_block.ip_block.0.cidr_notation, 2 ), gateway = metal_reserved_ip_block.ip_block.0.gateway, netmask = metal_reserved_ip_block.ip_block.0.netmask, vlanid = var.vlan_id})
}
